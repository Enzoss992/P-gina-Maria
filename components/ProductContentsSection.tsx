...
const ProductContentsSection: React.FC = () => {
  const items = [
    ...
    "Guia completo para uma caminhada com Cristo",
  ...
  return (
    <section className="py-12 bg-[#001021] flex flex-col items-center px-4 overflow-hidden">
      ...
      <div className="bg-[#ff3333] text-white px-8 py-3 rounded-full shadow-[0_0_20px_rgba(255,51,51,0.4)] mb-8 flex items-center justify-center gap-2 max-w-[90%] md:max-w-none">
      ...
      {/* Mockup Container Sem Borda/Fundo - Atualizado conforme solicitação */}
      <div className="w-full max-w-2xl mb-10 relative flex justify-center">
        <img 
          src="https://i.imgur.com/8squ2g7.png" 
          alt="Mockup do Produto" 
          className="w-full h-auto drop-shadow-[0_25px_40px_rgba(0,0,0,0.3)]"
          loading="lazy"
        />
      </div>
      ...
      <div className="w-full max-w-xl bg-black/40 border border-red-600/30 rounded-[2.5rem] p-8 md:p-10 relative mt-4 shadow-[0_0_50px_rgba(255,0,0,0.1)]">
        ...
        <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-[#2dd461] text-black px-6 py-1 rounded-full flex items-center gap-2 shadow-lg border border-white/20">
          <span className="text-sm">⚡</span>
          <span className="text-[11px] font-bold uppercase tracking-widest">ACESSO IMEDIATO</span>
        </div>
        ...
        <div className="space-y-6">
          {items.map((item, idx) => (
            <div key={idx} className="flex items-start gap-4 pb-4 border-b border-white/5 last:border-0 last:pb-0">
              ...
              <p className="text-white/90 text-sm md:text-lg font-bold leading-tight">
                {item}
              </p>
            </div>
          ))}
